...
describe("test", () => {
  it("test createProfileWithHandleUsingCredits", async () => {
    const signers = await ethers.getSigners();
    const signer = signers[1];

    console.log(signer.address);
    const HeyLensSignupV2 = await ethers.getContractFactory("HeyLensSignupV2");
    const hetCon = await HeyLensSignupV2.attach(
      "0x038D24604118BA083bF1BBe4Cb784D7923FDFb94"
    );
    try {
      const tx = await hetCon.callStatic.createProfileWithHandleUsingCredits(
        [
  ...
  it("test setHandleLengthMin", async () => {
    // 合约的 ABI 和地址
    ...
    const tx = await leHub.setHandleLengthMin(5);
    await tx.wait();
    console.log(tx.hash);
    const getHandleLengthMin = await leHub.getHandleLengthMin();
    await tx.wait();
    console.log(getHandleLengthMin);
  });

  it("test addCreditProvider", async () => {
    // 合约的 ABI 和地址
    const contractAddress = "0x77B2252d6c2ffEFC11CbD17308d840bA12b9BDe9";
    ...
    const tx = await leHub.addCreditProvider(signer.address);
    await tx.wait();
    console.log(tx.hash);
  });

  it("test setTrustStatus", async () => {
    // 合约的 ABI 和地址
    const contractAddress = "0x77B2252d6c2ffEFC11CbD17308d840bA12b9BDe9";
    ...
    const tx = await leHub.setTrustStatus(
      "0x038D24604118BA083bF1BBe4Cb784D7923FDFb94",
      false
    );
    await tx.wait();
  ...
  it("test increaseCredits", async () => {
    ...
    console.log(
      await leHub.isUntrusted("0x038D24604118BA083bF1BBe4Cb784D7923FDFb94")
    );

    // _isUntrusted[account] ==false
    console.log(
      await leHub.getCreditBalance("0x038D24604118BA083bF1BBe4Cb784D7923FDFb94")
    );

    const tx = await leHub.increaseCredits(
      "0x038D24604118BA083bF1BBe4Cb784D7923FDFb94",
      2
    );
    await tx.wait();
  ...
  it("test whitelistProfileCreator", async () => {
    // 合约的 ABI 和地址
    ...
    // 使用 ABI 和地址创建合约实例
    const leHub = new ethers.Contract(contractAddress, xAbi, signer);
    const data = targetInterface.encodeFunctionData("whitelistProfileCreator", [
      "0x77B2252d6c2ffEFC11CbD17308d840bA12b9BDe9",
      true
    ]);
    console.log(await leHub.controllerContract());
    const tx = await leHub.executeAsGovernance(
      "0x850CFD1FBDBb7221832C5d35385b1c4b4c06Dc5f",
      data
    );
    await tx.wait();
    console.log(tx);
