...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const response = await fetch(
    "https://graph-node.ctblock.cn/subgraphs/name/lens-grpah",
    {
      body: JSON.stringify({
        operationName: "Profile",
        query: print(ProfileDocument),
        variables: { fullHandle: `${HANDLE_PREFIX}${handle}` }
      }),
      cache: "no-store",
  ...
  return {
    alternates: { canonical: `https://lcm.xyz${link}` },
    ...
    metadataBase: new URL(`https://lcm.xyz${link}`),
    openGraph: {
      description: description,
...
export default async function Page({ params }: Props) {
  ...
  const profileUrl = `https://lcm.xyz/u/${metadata.other?.["lens:handle"]}`;
