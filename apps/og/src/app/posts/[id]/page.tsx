...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const publication = data.data.publication as Publication;
  ...
  return {
    alternates: { canonical: `https://lcm.xyz/posts/${targetPublication.id}` },
    applicationName: APP_NAME,
    authors: {
      name: displayName,
      url: `https://lcm.xyz${link}`
    },
    creator: displayName,
    ...
    metadataBase: new URL(`https://lcm.xyz/posts/${targetPublication.id}`),
    openGraph: {
      description: description,
      images: getPublicationOGImages(metadata) as any,
      siteName: "Lcm",
      type: "article",
      url: `https://lcm.xyz/posts/${targetPublication.id}`
    },
    other: {
...
export default async function Page({ params }: Props) {
  ...
  const postUrl = `https://lcm.xyz/posts/${metadata.other?.["lens:id"]}`;
