...
import { LCM_MEMBERSHIP_NFT_PUBLICATION_ID } from "@hey/data/constants";
...
export const get = [
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (cachedData) {
        logger.info(
          `(cached) Lcm NFT badge fetched for ${id || formattedAddress}`
        );
      ...
      const data = await lensPg.query(
        ...
        `,
        [id, formattedAddress, LCM_MEMBERSHIP_NFT_PUBLICATION_ID]
      );
      ...
      logger.info(`Lcm NFT badge fetched for ${id || formattedAddress}`);
