...
import { LCM_LENS_SIGNUP } from "@hey/data/constants";
...
export const get = [
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (cachedData === "true") {
        logger.info(
          `(cached) Lcm profile badge fetched for ${id || formattedAddress}`
        );
      ...
      const data = await lensPg.query(
        ...
        `,
        [id, formattedAddress, LCM_LENS_SIGNUP]
      );
      ...
      logger.info(`Lcm profile badge fetched for ${id || formattedAddress}`);
