...
import type {DistributiveOmit, UnknownObject} from "@lens-protocol/shared-kernel";
...
type ExtractPathsOf<
    ...
    By extends UnknownObject
> = PathsOf<Extract<Target, By>>;
...
type MapOfPaths<
    ...
    DiscriminantValue extends string = Target[Discriminant] extends string
        ...
        : never
> = {
    [P in DiscriminantValue]: ExtractPathsOf<
        Target,
        { [KK in Discriminant]: P }
    >[];
};
...
type EncryptablePath = PathsOf<
    DistributiveOmit<raw.PublicationMetadata, "encryptedWith">
>;

export function resolvePathsToEncrypt({
                                          $schema
                                      }: raw.PublicationMetadata): EncryptablePath[] {
...
type DecryptablePath = PathsOf<
    DistributiveOmit<gql.PublicationMetadata, "encryptedWith">
>;

const DecryptablePathsMapping: {
    [key in EncryptablePath]?: DecryptablePath | DecryptablePath[];
} = {
    ...
    "lens.attachments[n].item": "attachments[n].uri",
    "lens.attachments[n].lyrics": "attachments[n].lyrics",
    "lens.attachments[n].recordLabel": "attachments[n].recordLabel",
    "lens.audio.artist": "asset.audio.raw.artist",
    "lens.audio.cover": "asset.audio.raw.cover",
    ...
    "lens.video.cover": "asset.video.raw.cover",
...
export function resolvePathsToDecrypt({
    ...
    return encryptedWith.encryptedPaths
        .reduce((acc, path) => {
            if (
                path in DecryptablePathsMapping &&
                DecryptablePathsMapping[path as EncryptablePath]
            ) {
                acc.push(
                    DecryptablePathsMapping[path as EncryptablePath] as DecryptablePath
                );
            }
            return acc;
        }, [] as DecryptablePath[])
