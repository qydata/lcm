...
const Publications: FC<PublicationsProps> = ({query}) => {
    ...
    const useSearchPublicationsQuery = (param: any) => {
        let {data, error, fetchMore, loading} = {'data': "", 'error': "", 'fetchMore': "", 'loading': "",}
        return {data, error, fetchMore, loading}
    }
    const request: any = {
        limit: 25,
        query,
    ...
    const {data, error, fetchMore, loading} = useSearchPublicationsQuery({
        onCompleted: async ({searchPublications}: any) => {
