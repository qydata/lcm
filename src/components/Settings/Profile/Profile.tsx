import type {Area} from "@lcm/image-cropper/types";
...
import {AVATAR, COVER, LENS_HUB} from "@lcm/data/constants";
...
import trimify from "@lcm/helpers/trimify";
import {getCroppedImg} from "@lcm/image-cropper/cropUtils";
import {useProfileQuery} from "@lcm/lens";
import {Button, Card, ErrorMessage, Form, Image, Input, Modal, Spinner, TextArea, useZodForm} from "@lcm/ui";
    ...
    const editProfile = async (data: FormData) => {
        ...
        try {
            ...
            const otherAttributes =
                currentProfile.metadata?.attributes
                    ?.filter(
                        (attr) => !["location", "timestamp", "website"].includes(attr.key)
                    )
                    .map(({key, type, value}) => ({
                        key,
