...
import { LENS_HUB, ZERO_ADDRESS } from "@lcm/data/constants";
import { Modal } from "@lcm/ui";
...
const PublicationHeader: FC<PublicationHeaderProps> = ({
  ...
  const onProfileSelected = async (access_profile: Profile) => {
    ...
    const ori_metadata = await getFromIPFS(publication.metadata.id);

    const own_encryptionKey = ori_metadata.lens.encryptedWith.encryptionKey;
    //  解密
    ...
    const access_Key = await getEncryptionKey({
      priKey: privateKey,
      toDecrypt: own_encryptionKey
    });
    const _publicKey = access_profile.signInfo.publicKey.replace("0x", "");
    const access_encryptionKey = await saveEncryptionKey({
      pubkey: _publicKey,
      symmetricKey: uint8arrayToHexString(access_Key)
    });

    const _accessCondition = ori_metadata.lens.encryptedWith.accessCondition;
    for (const accessConditionKey in _accessCondition) {
      if (
        _accessCondition[accessConditionKey].address.toLowerCase() ===
        access_profile.ownedBy.toLowerCase()
      ) {
    ...
    const postParams = {
      ...
      contentURI:
        `SHARE://${publication.id}/${access_profile.id}/` +
        momokaRequest.contentURI,
      actionModules: [],
