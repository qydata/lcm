...
const generateAsymmetricKeyPair = async () => {
  ...
  return {
    publicKeyHex: Array.from(new Uint8Array(publicKey))
      .map((b) => b.toString(16).padStart(2, "0"))
      .join(""),
    privateKeyHex: Array.from(new Uint8Array(privateKey))
      .map((b) => b.toString(16).padStart(2, "0"))
      .join("")
  };
};
...
const upSignInfo = async (signature, profile) => {
  const { publicKeyHex, privateKeyHex } = await generateAsymmetricKeyPair();
  const encPrivateKeyHex = await aesEncryptionKey({
